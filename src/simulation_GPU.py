...
def fill_individual_transitions(prediction, transitions, size, seed):
    ...
    graph_suited, _ = net.check_graph_suitable(
        G=graph, starting_node=starting_transition
    )
    if not graph_suited:
        raise ValueError(
            "graph is not suited for the algorithm. Check for loops that do not "
    ...
    for i, transition in enumerate(transition_order):
        ...
        if starting_transition in follow_up_transitions:
            ...
            indices_not_starting_transition = cp.where(
                follow_up_transitions != starting_transition
            )[0]
            follow_up_transitions = follow_up_transitions[
                indices_not_starting_transition
            ]
            number_of_deletions = (
                rounded_repeats.sum()
                - rounded_repeats[indices_not_starting_transition].sum()
            )
        ...
        follow_up_transitions_torch = repeat_interleave(
            follow_up_transitions_torch, rounded_repeats
        )
        ...
        transition_series_new = cp.full(
            shape=(transition_series.size + insert_at.size), fill_value=-1
        )
        ...
        transition_series_new[cp.where(transition_series_new == -1)[0]] = (
            transition_series
        )
...
def direct_method_time(
    transition_matrix_sorted_indices,
    cumsum_sorted_trm,
    row_sums,
    start_index=0,
    size=10,
    ...
    while time_series[i] < end_time:
        ...
        if current_state_lambda == 0:
            ...
            break
        transition_time = (
            1 / current_state_lambda * cp.log(1 / random_numbers[i - (j - 1) * size, 0])
        )
        sorted_index = cp.searchsorted(
...
def direct_method_time_large_array(
    transition_matrix_sorted_indices,
    cumsum_sorted_trm,
    row_sums,
    start_index=0,
