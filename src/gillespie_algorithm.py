...
def direct_method_steps(transition_matrix, row_sums, n_steps, start_id, seed):
    """
    ...
    for i in range(n_steps):
        ...
        # Given an initial state x, all possible future states have the same exponentially distributed time tau as a
        # variable - this is due to the more states become available (no matter their rate constants), the state x will
        # be occupied for less time. This means - even if a future state y has a very low rate constant - if there are
        # other possible states with high rate constants, y will only occur if it happens rather quick - therefore, the
        # individual rate constants give an insight in the mean occupation time only if it is the only rate constant.
        ...
        state_series[i + 1] = current_state_index = future_state
...
def direct_method_time(transition_matrix, row_sums, size, start_id, end_time, seed):
    """
    The direct method of the gillespie algorithm (i.e., the stochastic simulation algorithm). Note that in this version,
    the propensities are equal to the rate constants, because the occupied state's population is assumed to be  always 1
    (the initial row vector contains only zeros except a single one). Additionally, the state change vector is also
    ...
    row_sums : np.ndarray
        ...
        joined state.
    size : int
        Size of random numbers to be drawn at once. Most efficient if drawn only once without overkill.
    start_id : int
        Id of the starting joined state.
    end_time : float
    ...
    time_step_series = []
    time_step_series.append(0)

    state_series = []
    current_state_index = np.where(current_state == 1)[0][0]
    state_series.append(current_state_index)
    ...
    if end_time is not None:
        ...
        while time_series[-1] < end_time:
            ...
            future_state = transition_matrix_sorted_indices[current_state_index, sorted_index]
            state_series.append(future_state)
