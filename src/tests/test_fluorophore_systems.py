...
def test_simulate_fluorophoresystem():
    ...
    goal_time_series, goal_time_step_series, goal_state_series = ga.direct_method_py(system.row_sums,
                                                                                     system.initial_row_vector,
                                                                                     system.transition_matrix, 100, 100)
...
def test_init_jablonskimodel():
    system = fc.JablonskiModel(2, 1, {"k_S0_S1": 0.4, "k_S1_T1": 5.8, "k_T1_S0": 1e-2, "k_T1_R": 13, "k_R_S0": 0.3},
                               induction_rate=4)
    assert system.number == 2
    assert system.distances == 1
    assert system.states == ("S0", "S1", "T1", "R", "B")
...
def test_emitters_jablonskimodel():
    system_1 = fc.JablonskiModel(2, 1, {"k_S0_S1": 0.4, "k_S1_T1": 5.8, "k_T1_S0": 1e-2, "k_T1_R": 13, "k_R_S0": 0.3,
                                        "k_S1_S0": 10},
                                 induction_rate=4)
    system_1.simulate()
    system_1.emitters(photon_collection=0.7, resample="5ms", unit="s", threshold=0, memory=0, use_unique=True)
    system_2 = fc.JablonskiModel(2, 1, {"k_S0_S1": 0.4, "k_S1_T1": 5.8, "k_T1_S0": 1e-2, "k_T1_R": 13, "k_R_S0": 0.3,
                                        "k_S1_S0": 10},
                                 induction_rate=4)
    system_2.simulate()
...
def test_init_onoffmodel():
    system = fc.OnOffModel(2, 1, {"k_ON_OFF": 5, "k_OFF_ON": 2, "k_ON_B": 0.01},
                           induction_rate=1)
    assert system.number == 2
    assert system.distances == 1
    assert system.states == ("ON", "OFF", "B")
    assert system.rates == {"k_ON_OFF": 5, "k_OFF_ON": 2, "k_ON_B": 0.01}
