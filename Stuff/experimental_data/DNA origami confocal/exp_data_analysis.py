...
def go_through_files(path, memory, threshold, skip_id=None):
    ...
    for j, filepath in enumerate(files):
        ...
        emissions.plot_time_series(axes=axes[i*5+0, 0])
        axes[i*5+0, 0].text(x=-0.2, y=1.1, s=f'index = {j}', 
                            transform=axes[i*5+0, 0].transAxes)
        emissions.plot_histogram(bins=30, axes=axes[i*5+0, 1], display_mean=True)
        axes[i*5+0, 1].text(x=0.8, y=0.9, s=f'# = {emissions.event_time_series[emissions.event_time_series > 0].size}',
                    transform=axes[i*5+0, 1].transAxes)
        blinks.plot(mode='off_histogram', axes=axes[i*5+1, 0], as_time='s')
        blinks.plot(mode='off_boxplot', axes=axes[i*5+1, 1], yscale='log', as_time='s')
        axes[i*5+1, 1].text(x=0.8, y=0.9, s=f'# (<10) = {blinks.off_periods[blinks.off_periods < 10].size}',
                            transform=axes[i*5+1, 1].transAxes)
        axes[i*5+1, 1].text(x=0.8, y=0.8, s=f'# (>10) = {blinks.off_periods[blinks.off_periods > 10].size}',
                            transform=axes[i*5+1, 1].transAxes)
        blinks.plot(mode='on_histogram', axes=axes[i*5+2, 0], as_time='ms')
        blinks.plot(mode='on_boxplot', axes=axes[i*5+2, 1], as_time='ms')
        axes[i*5+2, 1].text(x=0.8, y=0.9, s=f'# = {blinks.on_periods.size}',
                            transform=axes[i*5+2, 1].transAxes)
        blinks.plot(mode='on_frame_series', axes=axes[i*5+3, 0], yscale='log')
        blinks.plot(mode='off_frame_series', axes=axes[i*5+3, 1], yscale='log')
        rol_avrg_emissions = moving_average(emissions.event_time_series.values, n=5000)
        means = emissions.event_time_series.groupby(emissions.event_time_series.index // 1.0).apply(mean_without_zeros)
        fi.universal_figure(type_='line', data=[emissions.event_time_series.index[4999:], rol_avrg_emissions], axes=axes[i*5+4, 1],
                            ...
                            axes=axes[i*5+4, 0], title='nonzero intensities (every 1s)', xlabel='time (s)', ylabel='intensity')
        i+= 1
...
def sum_all_files(path, memory, threshold, skip_id=None):
    ...
    axes[0, 0].text(s='µ = {:.2e} s'.format(np.mean(np.concatenate(off_periods))), x=0.8, y=0.9,
                    transform=axes[0, 0].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    fi.universal_figure(type_='line', data=[np.arange(0, len(off_periods_small_10_num)), off_periods_small_10_num], 
                        axes=axes[0, 1], title='off periods < 10 frames', xlabel='files', ylabel='count')
    axes[0, 1].text(s='µ = {:.2e}'.format(np.mean(off_periods_small_10_num)), x=0.8, y=0.9,
                    transform=axes[0, 1].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    fi.universal_figure(type_='line', data=[np.arange(0, len(off_periods_large_10_num)), off_periods_large_10_num], 
                        axes=axes[1, 0], title='off periods > 10 frames', xlabel='files', ylabel='count')
    axes[1, 0].text(s='µ = {:.2e}'.format(np.mean(off_periods_large_10_num)), x=0.8, y=0.9,
                    transform=axes[1, 0].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    fi.universal_figure(type_='boxplot', data=on_periods, axes=axes[2, 0], yscale='log',
                        xlabel='files', ylabel='on period (ms)')
    axes[2, 0].text(s='µ = {:.2e} ms'.format(np.mean(np.concatenate(on_periods))), x=0.8, y=0.9,
                    transform=axes[2, 0].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    ...
    axes[3, 0].text(s='µ = {:.2e}'.format(np.mean(inten_means)), x=0.8, y=0.9,
                    transform=axes[3, 0].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    axes[3, 0].set_ylim(0, )
    ...
    axes[3, 1].text(s='µ = {:.2e}'.format(np.mean(inten_num)), x=0.8, y=0.9,
                    transform=axes[3, 1].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    fi.universal_figure(type_='line', data=[np.arange(0, len(off_periods_small_10_num)), off_periods_small_10_num], 
                        axes=axes[0,1],
    ...
    axes[2, 1].text(s='µ = {:.2e}'.format(np.mean(on_periods_num)), x=0.8, y=0.9,
                    transform=axes[2, 1].transAxes, bbox=dict(facecolor='white', edgecolor='black'))
    figure.tight_layout()
