...
def test_random_unit_vector():
    v = kappa_sq.random_unit_vector(size=1, seed=42)
    assert v.shape == (1, 3)
    assert np.allclose(np.linalg.norm(v, axis=1), 1)
    v = kappa_sq.random_unit_vector(size=10, seed=42)
    assert v.shape == (10, 3)
    assert np.allclose(np.linalg.norm(v, axis=1), 1)
...
def test_rotational_diffusion_step():
    ...
    dt = 1.0
    tau_rot = 10.0
    rotated_v = kappa_sq.rotational_diffusion_step(v, dt, tau_rot, seed=42)
...
def test_simulate_rotational_motion():
    ...
    traj1, traj2 = kappa_sq.simulate_rotational_motion(tau_rot, tau_life, dt, seed=42)
...
def test_sample_kappa_squared_distribution():
    ...
    size = 50
    samples = kappa_sq.sample_kappa_squared_distribution(k2_values, size=size, seed=42)
